...
def run(config_path, debug):
    ...
    syn_df = utils.load_pkl_df(config.syn_clustering_lookup_df_pklfname)
    ...
    for seed, assembly_grp in tqdm(assembly_grp_dict.items(), desc="Iterating over seeds"):
        ...
        target_gids = sim.circuit.cells.ids({"$target": config.syn_clustering_target})
        for assembly in tqdm(assembly_grp.assemblies, desc="%s syn. clusters" % seed, leave=False):
            gids = _get_degree_sorted_assembly_gids_within_target(sim.circuit, target_gids, conn_mat,
                                                                  assembly, mtypes, n_samples)
            syn_df_gids = syn_df.loc[syn_df["post_gid"].isin(gids)]
